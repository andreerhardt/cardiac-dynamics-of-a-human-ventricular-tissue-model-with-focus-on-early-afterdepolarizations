...
class Tentusscher_panfilov_2006_endo_14d(CardiacCellModel):
    ...
    def _I(self, v, s, time):
        ...
        V = v
        assert(len(s) == 13)
        Xr1, Xs, m, d, f, f2, fCass, s, Ca_i, R_prime, Ca_SR, Ca_ss, Na_i = s
        ...
        i_Kr = 0.430331482912*g_Kr*ufl.sqrt(K_o)*(-E_K + V)*Xr1*xr2_inf
        ...
        j_inf = 1.0/((1 + 15212.5932857*ufl.exp(0.134589502019*V))*(1 +\
            15212.5932857*ufl.exp(0.134589502019*V)))
        i_Na = g_Na*(m*m*m)*(-E_Na + V)*h_inf*j_inf
    ...
    def F(self, v, s, time=None):
        ...
        F_expressions[1] = (xs_inf - Xs)/tau_xs
        ...
        F_expressions[6] = (fCass_inf - fCass)/tau_fCass
        ...
        s_inf = 1.0/(1 + ufl.exp(28/5 + V/5))
        tau_s = 8+ 1000*ufl.exp(-(V+67)*(V+67)/1000)
        F_expressions[7] = (s_inf - s)/tau_s
        ...
        Ca_ss_bufss = 1.0/(1 + Buf_ss*K_buf_ss/((K_buf_ss + Ca_ss)*(K_buf_ss\
            + Ca_ss)))
        F_expressions[8] = (-Cm*(i_b_Ca - 2*i_NaCa + i_p_Ca)/(2*F*V_c) +\
            i_xfer + V_sr*(-i_up + i_leak)/V_c)*Ca_i_bufc
        k1 = k1_prime/kcasr
        k2 = k2_prime*kcasr
        O = (Ca_ss*Ca_ss)*R_prime*k1/(k3 + (Ca_ss*Ca_ss)*k1)
        F_expressions[9] = -Ca_ss*R_prime*k2 + k4*(1 - R_prime)
        i_rel = V_rel*(Ca_SR - Ca_ss)*O
        F_expressions[10] = (i_up - i_leak - i_rel)*Ca_sr_bufsr
        F_expressions[11] = (V_sr*i_rel/V_ss - Cm*i_CaL/(2*F*V_ss) -\
            V_c*i_xfer/V_ss)*Ca_ss_bufss

        # Expressions for the Sodium dynamics component
        F_expressions[12] = Cm*(-i_b_Na - 3*i_NaCa - 3*i_NaK - i_Na)/(F*V_c)
    ...
    def __str__(self):
        return 'Tentusscher_panfilov_2006_endo_cell_14d cardiac cell model'
