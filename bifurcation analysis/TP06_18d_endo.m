...
E_Ks=R*T/F*log((K_o+P_kna*Na_o)/(par_K_i+P_kna*kmrgd(18)));
...
%i_Na=par_g_Na*kmrgd(5)^3*v^2*(kmrgd(1)-E_Na);
...
%tau_v=0.25-2.24*v_inf*(-1+tanh(7.9+(0.07)*kmrgd(1)))^(-1);
u=1./(1+exp(-5*(kmrgd(1)+40)));
ah=(1-u).*0.057.*exp(-(kmrgd(1)+80)./6.8);
bh=0.77./(0.13.*(1+exp(-(kmrgd(1)+10.66)./11.1))).*u+(1-u).*(2.7.*exp(0.079.*kmrgd(1))+3.1.*10^5.*exp(0.3485.*kmrgd(1)));
tau_h=(ah+bh).^-1;
aj=(1-u).*((-2.5428.*10^4.*exp(0.2444.*kmrgd(1))-6.948.*10^-6.*exp(-0.04391.*kmrgd(1))).*(kmrgd(1)+37.78)./(1+exp(0.311.*(kmrgd(1)+79.23))));
bj=0.6.*exp(0.057.*kmrgd(1))./(1+exp(-0.1.*(kmrgd(1)+32))).*u+(1-u).*(0.02424.*exp(-0.01052.*kmrgd(1))./(1+exp(-0.1378.*(kmrgd(1)+40.14))));
tau_j=(aj+bj).^-1;
...
i_NaK=P_NaK*K_o/(K_o+K_mk)*kmrgd(18)/(kmrgd(18)+K_mNa)/(1+0.1245*exp(-0.1*kmrgd(1)*F/(R*T))+0.0353*exp(-kmrgd(1)*F/(R*T)));
