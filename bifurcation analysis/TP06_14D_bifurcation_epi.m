...
[t,y]   = ode15s(hfs{2},tspan,y0,opts,p_bif(1),p_bif(2),p_bif(3),p_bif(4),p_bif(5),p_bif(6));
...
figure;
hold on 
plotlimitcycle(xlc(:,1:1:end-200),vlc(:,1:1:end-200),slc,[size(xlc,1) q 1],[1 0 0],0.3,'none',0.2)
...
[x0,v0] = init_LC_LC(@TP06_epi_14d_bif, xlc, vlc, slc(end), par, ap, ntst, ncol);
opt = contset(opt,'MaxStepsize'        ,       10);
opt = contset(opt,'MinStepsize'        ,    1e-3);
opt = contset(opt,'MaxNewtonIters'     ,     150);
opt = contset(opt,'MaxCorrIters'       ,     150);
...
for i = 1:4
    i
    [xlc1,vlc1,slc1,hlc1,flc1] = cont(xlc1,vlc1,slc1,hlc1,flc1,cds);
end
...
plotlimitcycle(xlc1(:,1:1:end),vlc1(:,1:1:end),slc1,[size(xlc1,1) q 1],[0 0 1],0.3,':',0.2)
cpl_stability_codim1(xeq,seq,feq,q,1,[0 0 0],50,3)
cpl_stability_codim1(xeq1a,seq1a,feq1a,q,1,[0 0 0],50,3)
xlim([0.06 0.13])
view([155 5])

par=slc1(end).data.parametervalues;
[x0,v0] = init_LC_LC(@TP06_epi_14d_bif, xlc1, vlc1, slc1(end), par, ap, ntst, ncol);
opt = contset(opt,'MaxStepsize'        ,       10);
opt = contset(opt,'MinStepsize'        ,    1e-1);
opt = contset(opt,'MaxNewtonIters'     ,       3);
opt = contset(opt,'MaxCorrIters'       ,     150);
opt = contset(opt,'MaxTestIters'       ,     150);
opt = contset(opt,'Singularities'      ,       0);
opt = contset(opt,'Backward'           ,       1);
opt = contset(opt,'FunTolerance'       ,    1e-2); 
opt = contset(opt,'VarTolerance'       ,    1e-2); 
opt = contset(opt,'TestTolerance'      ,    1e-2);   
[xlc2,vlc2,slc2,hlc2,flc2]=cont(@limitcycle,x0,v0,opt);

[xlc2,vlc2,slc2,hlc2,flc2] = cont(xlc2,vlc2,slc2,hlc2,flc2,cds);
...
hold on 
plotlimitcycle(xlc(:,1:1:end),vlc(:,1:1:end),slc,[size(xlc,1) q 1],[1 0 0],0.3,'none',0.2)
plot3(xlc(end,slc(2).index)*xlc(q:dim:end-2,slc(2).index)./xlc(q:dim:end-2,slc(2).index),xlc(q:dim:end-2,slc(2).index),xlc(1:dim:end-2,slc(2).index),'-','Color',[0 0 1],'Linewidth',3);
plot3(xlc(end,slc(3).index)*xlc(q:dim:end-2,slc(3).index)./xlc(q:dim:end-2,slc(3).index),xlc(q:dim:end-2,slc(3).index),xlc(1:dim:end-2,slc(3).index),'-','Color',[0 1 1],'Linewidth',3);
plotlimitcycle(xlc1(:,1:1:end),vlc1(:,1:1:end),slc1,[size(xlc1,1) q 1],[1 0 0],0.3,'none',0.2)
plotlimitcycle(xlc2(:,1:1:end),vlc2(:,1:1:end),slc2,[size(xlc2,1) q 1],[0 0 0],0.3,'none',0.2)
cpl_stability_codim1(xeq,seq,feq,q,1,[0 0 0],50,3)
cpl_stability_codim1(xeq1a,seq1a,feq1a,q,1,[0 0 0],50,3)
xlim([0.06 0.13])
view([155 5])
