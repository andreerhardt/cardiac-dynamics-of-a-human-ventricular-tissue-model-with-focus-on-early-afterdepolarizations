...
function dydt = fun_eval(t,y,par_g_Kr,par_g_Ks,par_g_Na,par_g_K1,par_g_CaL,par_K_i,choice)
%% variables
V       = y(1); 
Xr1     = y(2); 
Xr2     = y(3); 
...
bh          = 0.77./(0.13.*(1+exp(-(V+10.66)./11.1))).*u+(1-u).*(2.7.*exp(0.079.*V)+3.1.*10^5.*exp(0.3485.*V));
tau_h       = (ah+bh).^-1;
aj          = (1-u).*((-2.5428.*10^4.*exp(0.2444.*V)-6.948.*10^-6.*exp(-0.04391.*V)).*(V+37.78)./(1+exp(0.311.*(V+79.23))));
